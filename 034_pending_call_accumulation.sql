-- Keep every unfinished call visible until it is completed or cancelled.
-- fechar_fila_do_dia marks missed suggestions as vencida; that status is an
-- urgency signal, not a reason to remove the task from future queues.
